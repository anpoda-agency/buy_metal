...
abstract class WriteLogs {
  static Future<void> write(String text) async {
    ...
    if (AppConstants.writeLogs) {
      String res = '';
      try {
        List<Directory>? pathLoc = await getExternalStorageDirectories(type: StorageDirectory.documents);
        final String path = (pathLoc != null) ? pathLoc[pathLoc.length - 1].path : '/storage/sdcard/0/Download';
