...
const List<String> suppliers = <String>['ООО "МЕТАЛЛ"', '*Поставщик 2', '*Поставщик 3', '*Поставщик 4',
...
class _SuppliersListPageState extends State<SuppliersListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.black87,title: const Center(child: Text('Перечень Поставщиков'),),),
      body: Container(
        child: ListView.builder(itemCount: suppliers.length,
        itemBuilder: (BuildContext context, index) {
          return Padding(
            ...
            child: InkWell(onTap: () {Navigator.pushNamed(context, '/supplier_proposals_list_page');}, 
            child: Padding(
              padding: const EdgeInsets.all(4.0),
              child: Align(alignment: Alignment.centerLeft,child: Text(suppliers[index], style: TextStyle(fontSize: 25, color: Colors.white),)),
            ),),
            ),
          ); } ),
      ),     
